...
void Scene::Plane::init_visualization()
{
	...
	glm::vec3 center(m_value * glm::vec3(abs(m_normal.x),abs(m_normal.y),abs(m_normal.z)));
...
{//m_value is D where xA + yB + zC = D
    float v1, v2;
    v1 = glm::dot(p1, m_normal) - m_value; //Is basically the plane formula if p1 is on the plane then v1 = 0
...
void Scene::Sphere::init_visualization()
{
    ...
    glm::vec3 mat_color(0.6f, 0.6f, 0.6f);
...
{
	// implement line-sphere intersection. you can refer to line-plane intersection.
    float v1, v2; // v1, v2 are distance to sphere for p1 and p2.
    v1 = glm::length(m_center - p1) - m_radius;
    v2 = glm::length(m_center - p2) - m_radius;
	glm::vec3 ray = glm::normalize(p2 - p1);  
	float a, b, c, t_int, radius, tmp; 
    if(v2 < threshold)
    {
        if(v1 >= threshold)
        {
			// continuous collision handling.
			radius = m_radius + threshold; 
			b = 2.0f * glm::dot(ray, (p1 - m_center)); 
			c = glm::dot((p1 - m_center), (p1 - m_center)) - pow(radius, 2.0f); 
			tmp = pow(b, 2.0f) - 4.0f * c; 
			if (tmp < 0) 
				return false; 
			t_int = (-1.0f * b - sqrt(tmp)) / (-2.0); 
            intersect = p1 + t_int * ray; 
        ...
        else
        {
			// static collision handling.
			radius = m_radius + threshold;  
            intersect = m_center + radius * glm::normalize(p1 - m_center);
			normal = glm::normalize(intersect - m_center); 
        }
        return true;
    }
...
//----------Mesh Class----------//
/*void Scene::Mesh::init_visualization()
{
    m_positions.clear();
    m_colors.clear();
    m_normals.clear();
...
void Scene::Mesh::draw(const VBO& vbos) const
{
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);                                                                                               
    // position
    glBindBuffer(GL_ARRAY_BUFFER, vbos.m_vbo);
...
bool Scene::Mesh::line_intersection(const glm::vec3& p1, const glm::vec3& p2, float threshold, glm::vec3& intersect, glm::vec3& normal) const
{
	// implement line-sphere intersection. you can refer to line-plane intersection.
    float v1, v2; // v1, v2 are distance to sphere for p1 and p2.
    v1 = glm::length(m_center - p1) - m_radius;
