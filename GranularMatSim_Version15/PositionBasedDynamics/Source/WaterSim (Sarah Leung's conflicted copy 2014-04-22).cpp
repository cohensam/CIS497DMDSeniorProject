...
WaterSim::WaterSim() {
	...
	// change to g/cm^3: 1000 kg (1000g/kg) / m^3 (100^3 cm^3/m^3) = 1 g/cm^3
	// particle has volume: (4/3)pi*0.1^3 = 0.004189
	// particle has volume: (4/3)pi*0.05^3 = 0.0005236
	// mass = density * volume = 0.5236
	rho_0 = 1000; 
}
...
void WaterSim::initialize() {
	...
	int pos = 0;
	for (float i = 0; i < /*4*//*3.2*/2*10*particleRad; i+=(2*particleRad)) 
	{
		for (float j = 0; j < /*4*//*3.2*/2*10*particleRad; j+=(2*particleRad))
		{
			for (float k = 0; k < /*4*//*3.2*/2*10*particleRad; k+=(2*particleRad))
			{
				particleList.pos(pos) = glm::vec3(i+0.95f,j+0.15f,k+0.95f);
				pos++;
			}
		}
	}
...
float WaterSim::calculateLambda(int id) {
	...
	{
		nbrid = Ni[j]; 
		dist = glm::length(particleList.predicted_pos(id)-particleList.predicted_pos(nbrid)); 
	...
	{
		nbrid = Ni[j]; 
		glm::vec3 diff = particleList.predicted_pos(id)-particleList.predicted_pos(nbrid); 
		delWj = kernelPressureDeriv(diff,nbrRadius); 
...
float WaterSim::kernelViscosity(float r, float h) {
	if (0 <= r && r <= h) {
		return 15.0f/(2.0f*PI*pow(h,3)) * (-(pow(r,3)/(2.0f*pow(h,3))) + (pow(r,2)/(pow(h,2))) + (h/(2*r)) - 1.0f);
	} else {
		return 0;
	}
}
